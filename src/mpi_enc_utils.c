...
MPP_RET mpi_enc_test_cmd_update_by_args(MpiEncTestArgs *cmd, int argc, char **argv)
{
    ...
    cmd->file_output = (char *)malloc(strlen(outfile) + 1);
    strcpy(cmd->file_output, outfile);
    ...
    if (cmd->rc_mode == MPP_ENC_RC_MODE_BUTT)
        cmd->rc_mode = (cmd->type == MPP_VIDEO_CodingMJPEG) ? MPP_ENC_RC_MODE_FIXQP : MPP_ENC_RC_MODE_VBR;
